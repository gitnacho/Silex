...
.. note::
    ...
    Cuando invocas a ``get()``, ``match()``, o cualquier otro método *HTTP* de la aplicación, de hecho, los estás llamando en una instancia predeterminada de ``ControllerCollection`` (almacenada en ``$app['controllers']``).

Otra ventaja es la capacidad para aplicar muy fácilmente la configuración a un conjunto de controladores. Basándote en el ejemplo de la sección del semiware, aquí tienes cómo protegerías todos los controladores para la colección ``backend``::
